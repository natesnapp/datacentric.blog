...
<template lang="md">
  ...
  In the last lesson we talked about the goal of Machine Learning which is to *devise a hyperplane for predicting and labeling new data*. The examples we have looked at so far are quite simple with a class having all its points separated by a straight line from those of another class. What happens when the data can not be so cleanly separated? Here is an example.

<img class="col-md-8" src="./imgs/point-cloud-2color-decision-tree.png" /><br clear="all" />

  In this plot, any single hyperplane will create zones that are *impure*, meaning that their point clouds will be represented by multiple labels with nearly equal probability. This is no good, and in order to break these apart, we now introduce the concept of Decision Trees. These allow the visual depiction of criteria for separating data based on features. In fact, they are actually good at using multiple combinations of features and are intuitive to describe. To create a decision tree on the example data set, we are going to define values of each feature that will separate point clouds into zones or regions. The goal is to have each region represented by a single label. To the extent that this is possible within the region, we call it *pure*.

  As we make these decisions at each branch in the tree, these will be performed by selecting some feature value that divides regions; we finally get to the leaf which will represent the label within the region itself. Each decision can use different features, which makes the model very robust, however only one feature can be used for each level in the tree. Let's take the first cut we want to make on our plot. In this example we are using the greek letter &Theta; to represent the criteria for each layer within the tree. So for instance, the first dec
...
<img class="col-md-12" src="./imgs/point-cloud-2color-decision-tree.png" /><br>
  ...
  <br>
<div><img class="col-md-8" src="./imgs/point-cloud-2color-decision-tree.png" /><div class="col-md-4"><ul><li ><em>Data points</em></li><li><em>Labels</em></li><li><em>Features</em></li><li><em>Separating hyper plane</em></li></ul></div></div><br clear="all" />
  ...
  <img class="col-md-8" src="./imgs/point-cloud-2color-decision-tree.png" /><br>
  ...
  **Unsupervised Learning:** K-means, mean shift, PCA, ICA
  ...
  <div><img class="col-md-8" src="./imgs/point-cloud-2color-decision-tree.png" /><div class="col-md-4"><img src="./imgs/point-cloud-2color-decision-tree.png"></div>What features would you choose for this data set?</div><br clear="all" />
  ...
  We are going to work with a dataset representing Google cluster scheduling and queuing behavior. <a href="https://drive.google.com/file/d/1lGbjR0Gs9Zuu0MCHPgVGdVM2KKjlEN16/view?usp=sharing">Download the dataset</a>, and make it available to load in your docker. Do this by putting it in the directory from where you start the docker instance. This folder loads as the /srv mount path and then you can call `.read_csv('/srv/google-borg-queued-jobs-cluster-h.csv')`, in your python notebook to load it..
